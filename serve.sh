...
ngrok http ${PORT} --log=stdout > /tmp/ngrok-kubernetes-labs.log 2>&1 &
NGROK_PID=$!
...
if [ -z "${URL}" ]; then
  echo "ERROR: ngrok tunnel did not come up. Check /tmp/ngrok-kubernetes-labs.log"
  exit 1
fi

echo ""
echo "==============================================="
echo "  Kubernetes + SRE Labs docs — public URL:"
echo "  ${URL}"
echo "==============================================="
echo ""
